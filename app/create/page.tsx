...
function CreatePageContent() {
  ...
  const secureToken = revokeMint || revokeFreeze;
  ...
  const uploadMetadataToIPFS = async () => {
    ...
    const res = await fetch("https://api.pinata.cloud/pinning/pinJSONToIPFS", {
      ...
      body: JSON.stringify({ pinataContent: metadata, pinataMetadata: { name: `metadata-${tokenName}` } }),
    });
    const data = await res.json();
  ...
  const createToken = async () => {
    ...
    try {
      ...
      // Metaplex metadata (ayrı transaction, kullanıcı imzalasın)
      let metadataSignature = null;
      if (metadataUri) {
        setStep("📝 Preparing metadata...");
        ...
        const metadataTxBytes = umi.transactions.serialize(metadataTx);
        const metadataTransaction = Transaction.from(Buffer.from(metadataTxBytes));
        metadataTransaction.recentBlockhash = blockhash;
        metadataTransaction.feePayer = publicKey;
        
        setStep("📝 Please sign metadata transaction...");
        metadataSignature = await sendTransaction(metadataTransaction, connection);
      }
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 sm:p-8 lg:sticky lg:top-28 space-y-4">
              <div className="text-center">
                ...
                <div className="mt-3 space-y-1.5 text-sm">
                  <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">{t("create_launch_fee")}</span><span className="font-bold text-green-600">{validReferrer ? "0.10 SOL" : "0.15 SOL"}</span></div>
                  <div className="border-t dark:border-gray-700 pt-2 flex justify-between font-semibold"><span className="text-gray-700 dark:text-gray-300">{t("create_total_fee")}</span><span className="text-green-600 font-bold text-lg">{validReferrer ? "0.10 SOL" : "0.15 SOL"}</span></div>
                  {validReferrer && <div className="text-xs text-green-500 mt-1">🎉 Referral active! You saved 0.05 SOL</div>}
                </div>
              </div>
              ...
              {mounted && (
                <button onClick={createToken} disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 disabled:opacity-50 text-white font-bold py-4 rounded-xl shadow-lg transition text-sm sm:text-base">
              ...
              {isProcessing && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                  <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-2 overflow-hidden"><motion.div className="bg-gradient-to-r from-blue-500 to-blue-600 h-full rounded-full" style={{ width: `${progress}%` }} transition={{ duration: 0.3 }} /></div>
                  <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400"><span>{step}</span><span className="font-mono text-blue-600">{Math.floor(progress)}%</span></div>
                </motion.div>
              )}
